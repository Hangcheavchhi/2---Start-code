...
function getCartTotalAmount() {
  ...
  SHOPPING_CART.forEach( element => {
    result += element.quantity * PRODUCTS[element.id - 1].price;
  });
  return result;
}
...
function addProductToCart(productId) {
  exist = SHOPPING_CART.find(element => element.id === productId)
  if (exist){
    exist.quantity++;
  }
  else{
...
function removeProductFromCart(productId) {
  removeItem = SHOPPING_CART.find(item => item.id === productId)
  if (!removeItem){
    return;
  }
    ...
    SHOPPING_CART.splice(SHOPPING_CART.indexOf(removeItem), 1);
  }
}
